...
class DecimalSetKeyValueDto {
  ...
  static DecimalSetKeyValueDto? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "DecimalSetKeyValueDto[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "DecimalSetKeyValueDto[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<DecimalSetKeyValueDto>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <DecimalSetKeyValueDto>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = DecimalSetKeyValueDto.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, DecimalSetKeyValueDto> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = DecimalSetKeyValueDto.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<DecimalSetKeyValueDto>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<DecimalSetKeyValueDto>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = DecimalSetKeyValueDto.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
