...
class BooleanKeyValue {
  ...
  static BooleanKeyValue? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "BooleanKeyValue[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "BooleanKeyValue[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<BooleanKeyValue>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <BooleanKeyValue>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = BooleanKeyValue.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, BooleanKeyValue> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = BooleanKeyValue.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<BooleanKeyValue>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<BooleanKeyValue>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = BooleanKeyValue.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
