...
class DateTimeKeyValue {
  ...
  static DateTimeKeyValue? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "DateTimeKeyValue[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "DateTimeKeyValue[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<DateTimeKeyValue>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <DateTimeKeyValue>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = DateTimeKeyValue.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, DateTimeKeyValue> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = DateTimeKeyValue.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<DateTimeKeyValue>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<DateTimeKeyValue>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = DateTimeKeyValue.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
