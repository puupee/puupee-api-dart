...
class FeaturesApi {
  ...
  Future<Response> apiFeatureManagementFeaturesPutWithHttpInfo({ String? providerName, String? providerKey, UpdateFeaturesDto? body, }) async {
    // ignore: prefer_const_declarations
    final path = r'/api/feature-management/features';
  ...
  ///
  /// * [UpdateFeaturesDto] body:
  Future<void> apiFeatureManagementFeaturesPut({ String? providerName, String? providerKey, UpdateFeaturesDto? body, }) async {
    final response = await apiFeatureManagementFeaturesPutWithHttpInfo( providerName: providerName, providerKey: providerKey, body: body, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
  }
