...
class AccountApi {
  ...
  Future<Response> apiAccountVerifyPasswordResetTokenPostWithHttpInfo({ VerifyPasswordResetTokenInput? verifyPasswordResetTokenInput, }) async {
    // ignore: prefer_const_declarations
    final path = r'/api/account/verify-password-reset-token';

    // ignore: prefer_final_locals
    Object? postBody = verifyPasswordResetTokenInput;
  ...
  Future<bool?> apiAccountVerifyPasswordResetTokenPost({ VerifyPasswordResetTokenInput? verifyPasswordResetTokenInput, }) async {
    final response = await apiAccountVerifyPasswordResetTokenPostWithHttpInfo( verifyPasswordResetTokenInput: verifyPasswordResetTokenInput, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
