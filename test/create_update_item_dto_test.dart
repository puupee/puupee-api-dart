//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:puupee_api/api.dart';
import 'package:test/test.dart';

// tests for CreateUpdateItemDto
void main() {
  // final instance = CreateUpdateItemDto();

  group('test CreateUpdateItemDto', () {
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // String name
    test('to test the property `name`', () async {
      // TODO
    });

    // String title
    test('to test the property `title`', () async {
      // TODO
    });

    // bool isHidden
    test('to test the property `isHidden`', () async {
      // TODO
    });

    // String description
    test('to test the property `description`', () async {
      // TODO
    });

    // String password
    test('to test the property `password`', () async {
      // TODO
    });

    // String parentId
    test('to test the property `parentId`', () async {
      // TODO
    });

    // String key
    test('to test the property `key`', () async {
      // TODO
    });

    // String md5
    test('to test the property `md5`', () async {
      // TODO
    });

    // String sliceMd5
    test('to test the property `sliceMd5`', () async {
      // TODO
    });

    // String rapidCode
    test('to test the property `rapidCode`', () async {
      // TODO
    });

    // String contentType
    test('to test the property `contentType`', () async {
      // TODO
    });

    // String type
    test('to test the property `type`', () async {
      // TODO
    });

    // String displayStyle
    test('to test the property `displayStyle`', () async {
      // TODO
    });

    // String extension_
    test('to test the property `extension_`', () async {
      // TODO
    });

    // String storageClass
    test('to test the property `storageClass`', () async {
      // TODO
    });

    // DateTime fileCreatedAt
    test('to test the property `fileCreatedAt`', () async {
      // TODO
    });

    // DateTime fileUpdatedAt
    test('to test the property `fileUpdatedAt`', () async {
      // TODO
    });

    // int syncVersion
    test('to test the property `syncVersion`', () async {
      // TODO
    });

    // bool isDeleted
    test('to test the property `isDeleted`', () async {
      // TODO
    });

    // DateTime deletionTime
    test('to test the property `deletionTime`', () async {
      // TODO
    });

    // DateTime creationTime
    test('to test the property `creationTime`', () async {
      // TODO
    });

    // DateTime lastModificationTime
    test('to test the property `lastModificationTime`', () async {
      // TODO
    });

    // int priority
    test('to test the property `priority`', () async {
      // TODO
    });

    // DateTime startAt
    test('to test the property `startAt`', () async {
      // TODO
    });

    // DateTime endAt
    test('to test the property `endAt`', () async {
      // TODO
    });

    // DateTime notifyAt
    test('to test the property `notifyAt`', () async {
      // TODO
    });

    // String notifyTimingType
    test('to test the property `notifyTimingType`', () async {
      // TODO
    });

    // String notifyTimingUnit
    test('to test the property `notifyTimingUnit`', () async {
      // TODO
    });

    // int notifyTimingValue
    test('to test the property `notifyTimingValue`', () async {
      // TODO
    });

    // String repeat
    test('to test the property `repeat`', () async {
      // TODO
    });

    // DateTime repeatOffAt
    test('to test the property `repeatOffAt`', () async {
      // TODO
    });

    // int repeatOffTimes
    test('to test the property `repeatOffTimes`', () async {
      // TODO
    });

    // bool isDone
    test('to test the property `isDone`', () async {
      // TODO
    });

    // DateTime doneAt
    test('to test the property `doneAt`', () async {
      // TODO
    });

    // String creatorId
    test('to test the property `creatorId`', () async {
      // TODO
    });

    // String lastModifierId
    test('to test the property `lastModifierId`', () async {
      // TODO
    });

    // String deleterId
    test('to test the property `deleterId`', () async {
      // TODO
    });

    // String tagging
    test('to test the property `tagging`', () async {
      // TODO
    });

    // String url
    test('to test the property `url`', () async {
      // TODO
    });

    // int size
    test('to test the property `size`', () async {
      // TODO
    });


  });

}
